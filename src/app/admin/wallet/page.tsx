...
export default function AdminWalletPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between"><h1 className="text-xl font-semibold text-gray-900">Wallet Management</h1><div className="flex gap-2"><button onClick={() => { resetForm(); setShowTopup(true) }} className="flex items-center gap-2 px-4 py-2.5 bg-green-600 text-white rounded-lg text-sm hover:bg-green-700 transition"><Plus size={16} /> Top Up</button><button onClick={() => { resetForm(); setShowDeduct(true) }} className="flex items-center gap-2 px-4 py-2.5 bg-red-600 text-white rounded-lg text-sm hover:bg-red-700 transition"><Minus size={16} /> Deduct</button><button onClick={() => { resetForm(); setShowAdjust(true) }} className="flex items-center gap-2 px-4 py-2.5 bg-yellow-600 text-white rounded-lg text-sm hover:bg-yellow-700 transition"><Wallet size={16} /> Adjust</button></div></div>
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map((w) => (
                <tr key={w.id} className="hover:bg-gray-50 transition">
                  <td className="px-4 py-3 font-medium text-gray-900">{w.user?.firstName} {w.user?.lastName}</td>
                  <td className="px-4 py-3 text-gray-600">{w.user?.email}</td>
                  <td className="px-4 py-3"><span className="px-2 py-0.5 rounded-full text-xs font-medium bg-blue-50 text-blue-700">{w.user?.role}</span></td>
                  <td className="px-4 py-3 text-right font-semibold text-gray-900">₹{Number(w.balance).toLocaleString("en-IN", { minimumFractionDigits: 2 })}</td>
                  <td className="px-4 py-3 text-xs text-gray-500">{new Date(w.updatedAt).toLocaleDateString()}</td>
                  <td className="px-4 py-3 text-right"><button onClick={() => openTransactions(w)} className="p-1.5 text-gray-400 hover:text-primary-600 rounded hover:bg-primary-50"><Eye size={14} /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {transactions.map((t) => (
                      <tr key={t.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-3 py-2"><span className={`px-2 py-0.5 rounded-full text-xs font-medium ${TYPE_COLORS[t.type] || "bg-gray-50 text-gray-700"}`}>{t.type}</span></td>
                        ...
                        <td className="px-3 py-2 text-right text-gray-900">₹{Number(t.balance).toLocaleString("en-IN", { minimumFractionDigits: 2 })}</td>
