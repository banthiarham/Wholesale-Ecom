...
export default function AdminBulkOrdersPage() {
  ...
  const loadBulkOrders = async () => {
    setLoading(true)
    try {
      const res = await fetch("/api/bulk-orders", { headers: { Authorization: `Bearer ${token}` } })
  ...
  const filtered = bulkOrders.filter((bo) => {
    const matchSearch =
      bo.bulkOrderNumber.toLowerCase().includes(search.toLowerCase()) ||
      bo.user?.email?.toLowerCase().includes(search.toLowerCase()) ||
      `${bo.user?.firstName} ${bo.user?.lastName}`.toLowerCase().includes(search.toLowerCase())
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map((bo) => (
                <>
                  <tr key={bo.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-4 py-3 text-right font-semibold text-gray-900">₹{Number(bo.totalAmount).toLocaleString("en-IN")}</td>
                    <td className="px-4 py-3">
                  ...
                  {/* Expanded detail row */}
                  {expandedId === bo.id && (
                    <tr key={`${bo.id}-detail`} className="bg-gray-50/50">
                            ...
                            <tbody className="divide-y divide-gray-100">
                              {bo.items.map((item) => (
                                <tr key={item.id}>
                                  ...
                                  <td className="px-3 py-2 text-right text-gray-900">₹{Number(item.unitPrice).toLocaleString("en-IN")}</td>
                                  <td className="px-3 py-2 text-center text-gray-900">{item.quantity}</td>
                                  <td className="px-3 py-2 text-right font-medium text-gray-900">₹{Number(item.totalPrice).toLocaleString("en-IN")}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
