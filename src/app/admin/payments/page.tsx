...
const providerColor: Record<string, string> = {
  CCAVENUE: "bg-blue-50 text-blue-700",
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          <div key={card.label} className="bg-white rounded-lg border border-gray-100 shadow-sm p-4">
            ...
            <p className="text-xs text-gray-500">{formatPrice(card.amount)}</p>
          </div>
        ))}
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="px-4 py-3 text-left font-medium text-gray-600">Order Status</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map((p) => (
                <tr key={p.id} className="hover:bg-gray-50 transition">
                  <td className="px-4 py-3 font-medium text-gray-900">#{p.order?.orderNumber?.slice(0, 8)}</td>
                  <td className="px-4 py-3 text-gray-600">
                    {p.order?.user ? (
                      <>
                        {p.order.user.firstName} {p.order.user.lastName}
                        <br />
                        <span className="text-xs text-gray-400">{p.order.user.email}</span>
                      </>
                    ) : "—"}
                  </td>
                  <td className="px-4 py-3 font-semibold">{formatPrice(Number(p.amount))}</td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded-full text-[11px] font-semibold ${providerColor[p.provider] || "bg-gray-100 text-gray-700"}`}>
                      {PROVIDER_LABELS[p.provider] || p.provider}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded-full text-[11px] font-semibold ${orderStatusColor[p.order?.status] || "bg-gray-100 text-gray-700"}`}>
                      {p.order?.status}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded-full text-[11px] font-semibold ${statusColor[p.status] || "bg-gray-100 text-gray-700"}`}>
                      {p.status}
                    </span>
                  </td>
          ...
          <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-xl max-h-[80vh] overflow-y-auto">
            ...
            <div className="space-y-3">
              <div className="bg-gray-50 rounded-lg p-3 text-sm space-y-1">
                <div className="flex justify-between"><span className="text-gray-600">Order:</span><span className="font-medium">#{selected.order?.orderNumber?.slice(0, 8)}</span></div>
                <div className="flex justify-between"><span className="text-gray-600">Customer:</span><span>{selected.order?.user ? `${selected.order.user.firstName} ${selected.order.user.lastName}` : "—"}</span></div>
                <div className="flex justify-between"><span className="text-gray-600">Email:</span><span className="text-xs">{selected.order?.user?.email || "—"}</span></div>
                <div className="flex justify-between"><span className="text-gray-600">Amount:</span><span className="font-medium">{formatPrice(Number(selected.amount))}</span></div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">Provider:</span>
                  <span className={`px-2 py-0.5 rounded-full text-[11px] font-semibold ${providerColor[selected.provider] || "bg-gray-100 text-gray-700"}`}>
                    {PROVIDER_LABELS[selected.provider] || selected.provider}
                  </span>
                </div>
                {selected.gateway && (
                  <div className="flex justify-between"><span className="text-gray-600">Gateway:</span><span className="text-xs">{selected.gateway.label}</span></div>
                )}
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">Status:</span>
                  <span className={`px-2 py-0.5 rounded-full text-[11px] font-semibold ${statusColor[selected.status] || "bg-gray-100 text-gray-600"}`}>{selected.status}</span>
                </div>
                {selected.providerRef && (
                  <div className="flex justify-between"><span className="text-gray-600">Ref:</span><span className="font-mono text-xs">{selected.providerRef}</span></div>
                )}
                <div className="flex justify-between"><span className="text-gray-600">Order Status:</span><span className={`px-2 py-0.5 rounded-full text-[11px] font-semibold ${orderStatusColor[selected.order?.status] || "bg-gray-100 text-gray-700"}`}>{selected.order?.status}</span></div>
              </div>
              <div className="border-t border-gray-100 pt-3">
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    <label className="block text-xs text-gray-500 mb-1">Provider Reference (optional)</label>
                    <input
                      type="text"
                      value={providerRef}
                  ...
                  <button
                    ...
                    className="w-full py-2.5 bg-primary-600 text-white rounded-lg text-sm hover:bg-primary-700 disabled:opacity-50"
                  >
