      ...
      {showForm && (
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm p-6">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <SearchableSelect options={products.map((p) => ({ value: p.id, label: p.title }))} value={form.productId} onChange={(v) => setForm({ ...form, productId: v, categoryId: v ? "" : form.categoryId })} placeholder="All Products" searchPlaceholder="Search products..." emptyText="No products found" />
            <SearchableSelect options={categories.map((c) => ({ value: c.id, label: c.name }))} value={form.categoryId} onChange={(v) => setForm({ ...form, categoryId: v, productId: v ? "" : form.productId })} placeholder="All Categories" searchPlaceholder="Search categories..." emptyText="No categories found" />
