        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-900 rounded-xl p-6 w-full max-w-md shadow-xl">
            ...
            <form onSubmit={handleEarn} className="space-y-4">
              <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">User</label><SearchableSelect options={users.map(u => ({ value: u.id, label: `${u.firstName} ${u.lastName}`, subtitle: u.email }))} value={form.userId} onChange={(value) => setForm({ ...form, userId: value })} placeholder="Select User" /></div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-900 rounded-xl p-6 w-full max-w-md shadow-xl">
            ...
            <form onSubmit={handleCashback} className="space-y-4">
              <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">User</label><SearchableSelect options={users.map(u => ({ value: u.id, label: `${u.firstName} ${u.lastName}`, subtitle: u.email }))} value={cbForm.userId} onChange={(value) => setCbForm({ ...cbForm, userId: value })} placeholder="Select User" /></div>
