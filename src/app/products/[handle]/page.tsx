...
import { ShoppingCart, ArrowLeft, Star, Truck, Shield, Package } from "lucide-react"
...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/store/products/${params.handle}`)
      .then((res) => res.json())
      .then((data) => {
        if (data.product) {
  ...
  if (!product) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center">
        ...
        <Link href="/products" className="mt-4 text-primary-600 hover:underline">
          Back to products
        </Link>
      </div>
    )
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
            {product.category && (
              <Link
                href={`/categories/${product.category.handle}`}
                  ...
                  <tbody>
                    {product.tierPrices.map((tp) => (
                      <tr
                        key={tp.id}
                        className={`border-b ${
                          quantity >= tp.minQty && (!tp.maxQty || quantity <= tp.maxQty)
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center border border-gray-200 rounded-lg">
                ...
                <button
                  onClick={() =>
                    setQuantity(Math.min(product.inventoryQuantity, quantity + 1))
                  }
                  className="px-3 py-2 hover:bg-gray-50"
                >+</button>
              </div>
              ...
              <button
                ...
                disabled={adding || product.inventoryQuantity <= 0 || quantity < product.moq}
